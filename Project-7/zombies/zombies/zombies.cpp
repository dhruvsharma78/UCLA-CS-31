...
class Zombie{
    ...
    private:
        ...
        int    m_row;
        int    m_col;
        int    m_health; // 0 = not injured, 1 = injured, 2 = dead
};
...
int Zombie::row() const{
    // Return row number of zombie
    return m_row;
}

int Zombie::col() const{
    // Return column number of zombie
    return m_col;
}
...
bool Zombie::getAttacked(int dir){  // return true if dies
    ...
    // Zombie dies if already has been hit or
    // if zombie is at an edge
    if((m_health == 1)            || (dir == 0 && m_row == 1) || (dir == 1 && m_row == m_arena->rows()) ||
        (dir == 2 && m_col == 1)  || (dir == 3 && m_col == m_arena->cols())) {
        m_health = 2;
        return true;
    }else{
        // Zombie loses one health point when hit
        m_health = 1;
...
void Player::moveOrAttack(int dir){
    ...
    // Temporarily store the location where player wants to move
    int rToMove = m_row;
    int cToMove = m_col;
    if(!(m_arena->determineNewPosition(rToMove, cToMove, dir))){
        return;
    }


    // Check if a zombie is at the position where player wants to move
    if(m_arena->numZombiesAt(rToMove,cToMove) > 0){
        // If a zombie exists, hit it and dont move player
        m_arena->attackZombieAt(rToMove,cToMove,dir);
    }else{
        m_arena->determineNewPosition(m_row, m_col, dir);
    }
}
...
int Arena::zombieCount() const{
    // Return number of zombies in arena
    return m_nZombies;
}
...
int Arena::numZombiesAt(int r, int c) const{
    ...
    for(int i = 0; i < zombieCount(); i++){
        // Check all zombies and seee if r and c matches
    ...
    for (r = 0; r < rows(); r++){
        for (c = 0; c < cols(); c++){
            if(numZombiesAt(r+1,c+1) >= 9)
                grid[r][c] = '9';
            else if(numZombiesAt(r+1,c+1) >= 2)
...
bool Arena::addZombie(int r, int c){
    ...
    // If no more zombies can be created return false
...
bool Arena::attackZombieAt(int r, int c, int dir){
    ...
    if(numZombiesAt(r,c) > 0){
        for(i = 0; i<zombieCount(); i++){
            // Check if there is a zombie at the position
            if(m_zombies[i]->row() == r && m_zombies[i]->col() == c){
                // Check if zombie dies from hit
                bool zombieDead = m_zombies[i]->getAttacked(dir);
                // If zombie dies, delete
                if(zombieDead){
                    delete m_zombies[i];
...
bool Arena::moveZombies(){
    for (int k = 0; k < m_nZombies; k++)
    {
        ...
        m_zombies[k]->move();
        // Check if zombie moves to players position
...
Game::~Game(){
    // Delete dynamically created pointers
    delete m_arena;
}
